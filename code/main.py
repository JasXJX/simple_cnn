...
def train(features_src: np.ndarray, labels_src: np.ndarray, model, ep_n, lr):
    features, labels = util.preprocess(torch.from_numpy(features_src)), \
                       util.preprocess(torch.from_numpy(labels_src))
...
if __name__ == "__main__":
    x_train, y_train, x_test, y_test = util.load_images("../code")
    model = util.CNN()
